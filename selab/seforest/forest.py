...
def _parallel_build_trees(
    ...
    sample_mask = _generate_sample_mask(tree.random_state, n_samples, n_samples_bootstrap)
    ...
    oob_prediction = _C_FOREST.predict(X[~sample_mask, :], feature, threshold, children, value)
    with lock:
...
class BaseForest(MultiOutputMixin, BaseEnsemble, metaclass=ABCMeta):
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        trees = [self._make_estimator(append=False, random_state=random_state) for i in range(self.n_estimators)]

        # Pre-allocate OOB estimations
        if is_classifier(self):
            oob_decision_function = np.zeros((n_samples, self.classes_[0].shape[0]))
        else:
        ...
        lock = threading.Lock()
        rets = Parallel(
            n_jobs=n_jobs, verbose=self.verbose, **_joblib_parallel_args(prefer="threads", require="sharedmem")
        )(
        ...
        if is_classifier(self) and (oob_decision_function.sum(axis=1) == 0).any():
            warn(
        ...
        if is_classifier(self):
            prediction = oob_decision_function / oob_decision_function.sum(axis=1)[:, np.newaxis]
        else:
