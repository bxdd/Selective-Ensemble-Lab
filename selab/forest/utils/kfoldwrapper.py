    ...
    def fit_transform(self, X, y, sample_weight=None):
        ...
        for k, (train_idx, val_idx) in enumerate(splitter.split(X, y)):
            ...
            if sample_weight is None:
                ...
                estimator.fit(X[train_idx], y[train_idx], sample_weight[train_idx])

            # Predict on hold-out samples
            if self.is_classifier:
                self.oob_decision_function_[val_idx] += estimator.predict_proba(X[val_idx])
            else:
        ...
        for estimator in self.estimators_:
            if self.is_classifier:
                ...
                if self.n_outputs > 1:
                    ...
                    out += estimator.predict(X).reshape(n_samples, -1)  # univariate regression
