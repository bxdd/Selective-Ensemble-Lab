...
def _find_binning_thresholds_per_feature(col_data, n_bins, bin_type="percentile"):
    """
    ...
    if len(col_data.shape) != 1:
        ...
        msg = "Per-feature data should be of the shape (n_samples,), but" " got {}-dims instead."
...
def _find_binning_thresholds(X, n_bins, bin_subsample=2e5, bin_type="percentile", random_state=None):
    ...
    for f_idx in range(n_features):
        threshold = _find_binning_thresholds_per_feature(X[:, f_idx], n_bins, bin_type)
...
class Binner(TransformerMixin, BaseEstimator):
    ...
    def _validate_params(self):
        ...
        if not 2 <= self.n_bins - 1 <= 255:
            msg = "`n_bins` should be in the range [2, 255], bug got" " {} instead."
            raise ValueError(msg.format(self.n_bins - 1))

        if not self.bin_subsample > 0:
            msg = (
                "The number of samples used to construct the Binner" " should be strictly positive, but got {} instead."
            )
            raise ValueError(msg.format(self.bin_subsample))

        if self.bin_type not in ("percentile", "interval"):
            msg = "The type of binner should be one of {{percentile, interval" "}}, bug got {} instead."
    ...
    def transform(self, X):
        ...
        if not self._is_fitted:
            msg = "The binner has not been fitted yet when calling `transform`."
            raise RuntimeError(msg)

        if not X.shape[1] == self.n_bins_non_missing_.shape[0]:
            msg = "The binner was fitted with {} features but {} features got" " passed to `transform`."
            raise ValueError(msg.format(self.n_bins_non_missing_.shape[0], X.shape[1]))
